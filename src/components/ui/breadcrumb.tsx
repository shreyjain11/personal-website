...
// Local minimal icons to avoid external deps
...
function BreadcrumbList({ className, ...props }: React.ComponentProps<"ol">) {
  return (
    <ol
      ...
      className={cn(
        "flex flex-wrap items-center gap-1.5 sm:gap-2.5 text-sm break-words text-foreground/70",
        className,
      )}
      {...props}
    />
...
function BreadcrumbSeparator({ children, className, ...props }: React.ComponentProps<"li">) {
  return (
    <li
      ...
      role="presentation"
      aria-hidden="true"
      className={cn("[&>svg]:h-3.5 [&>svg]:w-3.5 text-foreground/50", className)}
      {...props}
    >
...
function BreadcrumbEllipsis({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      ...
      role="presentation"
      aria-hidden="true"
      className={cn("flex w-9 h-9 items-center justify-center text-foreground/70 hover:text-foreground cursor-pointer", className)}
      {...props}
    >
