  ...
  if (loading) {
    return (
      <div className="w-full">
        ...
        <div className="text-foreground/60 text-sm">Loading repositories...</div>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-2 gap-3">
        {repos.map((repo) => (
          <a
            ...
            className="group p-3 rounded-lg bg-transparent hover:bg-foreground/10 backdrop-blur-sm transition-all duration-200 border border-foreground/10"
          >
            <div className="flex items-start justify-between gap-2">
              <span className="text-sm font-medium text-foreground group-hover:text-blue-300 transition-colors truncate">
                {repo.name}
              </span>
              {repo.stargazers_count > 0 && (
                <span className="flex items-center gap-1 text-xs text-foreground/60 shrink-0">
